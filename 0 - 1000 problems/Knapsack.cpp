...
int sol(const vector<pair<int, int>>& arr, int n, int k, int idx)
{
	if (idx != n)
	{
		int sum = arr[idx].first;
		int res = arr[idx].second;
		if (sum > k)
		{
			return sol(arr, n, k, idx + 1);
		}
		int x = res + sol(arr, n, k - sum, idx + 1);
		int y = sol(arr, n, k, idx + 1);
		return max(x, y);
	}
	return 0;
}
